...
def render_lane_cards(bundle: dict):
    ...
    for idx, lane in enumerate(bundle["lane_names"]):
        ...
        with cols[idx]:
            st.markdown(f'<div class="lane-card"><div class="lane-title">{lane}</div><div class="lane-sub">{len(lane_df)} studies captured · {len(active_lane)} active/near-active · {enroll:,} listed patients</div></div>', unsafe_allow_html=True)
